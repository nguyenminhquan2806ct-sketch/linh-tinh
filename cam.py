...
.n [delay] [@tags]                  : Spam từ file nhay.txt
.webhook [webhook] [noi_dung]       : Spam qua webhook
.tokenspam [id] [name_file] [delay] : Spam đa token
.tokentreodu [id] [du.txt] [delay]  : Spam treo đú
...
.stk               :hiện thông tin chuyển khoản
.store             :DVMXH
--- Hình Ảnh ---
...
@bot.command()
async def store(ctx):
    await ctx.send(f"""# 🔥 CHUYÊN CUNG CẤP TOOL & BOT DISCORD CHẤT LƯỢNG 🔥
## 📌 Hỗ trợ đầy đủ - Giá cả hợp lý - Cam kết chất lượng 📌
## 🎯 Các sản phẩm hot hiện có:
1️⃣ Tool Treo Nhây Đa Token
- Giá: 120K
**Tính năng vượt trội, hoạt động ổn định và hiệu quả.**
## 2️⃣ Tool Spam Đa Token
- Giá: 50K - 120K (tuỳ phiên bản).
**Hỗ trợ spam tin nhắn siêu lâu**
## 3️⃣ Bot War Discord
- V3: Giá 100K
- V4: Giá 150K
**Hỗ trợ chiến đấu mạnh mẽ, giao diện và tính năng tối ưu.**
## 4️⃣ Dịch vụ Tạo Bot Riêng
- Giá: 300K
- Tạo bot Discord theo yêu cầu, cá nhân hoá hoàn toàn, đáp ứng mọi nhu cầu của bạn!
## 5️⃣ Bán Source Tool/Bot
**Giá cả thương lượng.
Source code đầy đủ, dễ chỉnh sửa, tối ưu cao.**
**💎 Ưu điểm khi mua hàng:**
- ✔️ Hỗ trợ nhiệt tình trong quá trình sử dụng.
- ✔️ Bảo hành dài hạn, uy tín hàng đầu.
## 📩 Liên hệ ngay:
- Facebook: https://www.facebook.com/kenblack55
- Discord: nothingforme1.
## 📌 Cam kết:
- Hoàn tiền nếu sản phẩm không hoạt động đúng như cam kết.
- Hỗ trợ setup và hướng dẫn chi tiết !""")

@bot.command() 
async def stk(ctx):
    await ctx.send(f"""
- **Số tài Khoản 1: https://cdn.discordapp.com/attachments/1430101811033014367/1437000335100088360/IMG_2846.jpg?ex=6911a65a&is=691054da&hm=43876fa75912efe28ec7fd5a124d2d1c71a001b65bd03b2736259c627b67bfac""")
...
async def nuke(ctx, new_name: str, *, message: str):
    ...
    if ctx.guild.id in WHITELISTED_SERVERS:
        await ctx.send("**Máy chủ đang trong whitelist, không thể thực hiện!**")
        return
    guild = ctx.guild
    try:
...
async def du(ctx, delay: float):
    ...
    if spamming_nhay_task is not None:
        await ctx.send("Đang chạy du rồi")
        return
    ...
    try:
        ...
        await ctx.send("**Không tìm thấy tệp du.txt**")
        return
...
async def stopdu(ctx):
    ...
    if not active_features.get('du'):
        await ctx.send("**du đang không chạy**")
        return
